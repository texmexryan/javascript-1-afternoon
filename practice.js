...
//also works => return `Hello, ${name}`  ... cleaner than +
...
// difference of 'let' and 'const' => const cannot be changed, use if know wont need changing
//////////////////PROBLEM 3////////////////////
...
function doubleCheck(arr){
  if (arr.indexOf('chocolate')== -1){  // or if(arr.includes('chocolate'))
arr.push('chocolate')
  } return arr;
}
...
let devMountianClassPet = dog.name;
...
var arr = [2,3,4,25,100,115];
looper(arr);  //returns 243
...
var mathSum = math(3, 4, add);   // order matters cannot be (add, 3, 4)
...
function bathroom() {
  ...
  function bathtub() {
    let sailorDuck = "nautical";   // lexical scope => function will look inside itself first then global.
  }
}
